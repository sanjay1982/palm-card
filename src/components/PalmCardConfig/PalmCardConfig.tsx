    ...
    <div className={`${styles.PalmCardConfig} container d-print-none`} data-testid="PalmCardConfig">
      <form>
        <div className={`form-group row`}>
          <textarea className={`form-control`} id="text" aria-describedby="textHelp" placeholder="Enter text" value={text} 
          onChange={(e) => setText(e.target.value)} style={{minHeight: '300px' }}/>
          <small id="textHelp" className={`form-text text-muted`}>Text to break into palm cards.</small>
        </div>
        <div className={`form-group row`}>
          <div className={`col`}>
        ...
        <div className={`form-group row`}>
          <div className={`col`}>
            <button type="button" className={`btn btn-primary`} onClick={() => props.onComplete?.({ text, numberOfColumns, fontSize, maxTextLength })}>Submit</button>
          </div>
          <div className={`col`}>
